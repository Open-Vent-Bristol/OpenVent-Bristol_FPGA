...
"$SCRIPT_DIR/ghdl.sh" make   --workdir="$ROOT_DIR"/work --std=08 "$WARNINGS" ovb

echo "Running testbench"
"$SCRIPT_DIR/ghdl.sh" run    --workdir="$ROOT_DIR"/work --std=08 "$WARNINGS" ovb \
    --wave=ovb.ghw --stats --stop-time=100ms
