...
"$SCRIPT_DIR/ghdl.sh" make   --workdir="$ROOT_DIR"/work --std=08 "$WARNINGS" adc_tb

echo "Running testbench"
"$SCRIPT_DIR/ghdl.sh" run    --workdir="$ROOT_DIR"/work --std=08 "$WARNINGS" adc_tb \
    --wave=adc_tb.ghw --stats --stop-time=100ms
